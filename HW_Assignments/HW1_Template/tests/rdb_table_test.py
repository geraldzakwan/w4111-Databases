...
def test_find_by_primary_key(appearances_rdb):
    ...
    with pytest.raises(Exception):
        appearances_rdb.find_by_primary_key(None)
    with pytest.raises(Exception):
        appearances_rdb.find_by_primary_key([])
    with pytest.raises(Exception):
        appearances_rdb.find_by_primary_key(['1', '2'])
...
def test_find_by_template(appearances_rdb):
    ...
    with pytest.raises(Exception):
        assert appearances_rdb.find_by_template({'wrong': '150'}, ['playerID'])
    with pytest.raises(Exception):
        assert appearances_rdb.find_by_template({'playerID': 'aardsda01'}, ['wrong'])
    ...
    assert appearances_rdb.find_by_template({'G_all': '150', 'GS': '140', 'G_ph': '7'}, ['playerID', 'teamID', 'yearID']) == label
    ...
    # assert len(appearances_rdb.find_by_template({}, ['G_p'])) == 105789
...
def test_insert(appearances_rdb):
    ...
    assert appearances_rdb.find_by_primary_key(['aardsda01', 'ABC', '2015'], ['yearID', 'teamID', 'playerID']) == None
    # Insert that record
    assert appearances_rdb.insert({'yearID': '2015', 'teamID': 'ABC', 'lgID': 'NL', 'playerID': 'aardsda01', 'G_all': '33', 'GS': '0', 'G_batting': '30', 'G_defense': '33', 'G_p': '33', 'G_c': '0', 'G_1b': '0', 'G_2b': '0', 'G_3b': '0', 'G_ss': '0', 'G_lf': '0', 'G_cf': '0', 'G_rf': '0', 'G_of': '0', 'G_dh': '0', 'G_ph': '0', 'G_pr': '0'}) == None
    ...
    with pytest.raises(Exception):
        assert appearances_rdb.insert(None)
    with pytest.raises(Exception):
        assert appearances_rdb.insert({})
    with pytest.raises(Exception):
        assert appearances_rdb.insert({'playerID': 'geraldi'})
    with pytest.raises(Exception):
        assert appearances_rdb.insert({'wrong': '150'})
...
def test_delete_by_key(appearances_rdb):
    ...
    assert appearances_rdb.delete_by_key(['aardsda01', 'ABC', '2015']) == 0
    ...
    # Function fails if no key_fields are provided (None or empty) or if the dimension doesn't match with key_columns dimension
    with pytest.raises(Exception):
        appearances_rdb.delete_by_key(None)
    with pytest.raises(Exception):
        appearances_rdb.delete_by_key([])
    with pytest.raises(Exception):
        appearances_rdb.delete_by_key(['1', '2'])
...
def test_delete_by_template(appearances_rdb):
    # Delete fails, there is no matching row
    assert appearances_rdb.delete_by_template({'GS': '999'}) == 0
    # Count the number of records that match a specific template
    assert len(appearances_rdb.find_by_template({'GS': '150', 'G_all': '150'})) == 83
    # Delete them
    assert appearances_rdb.delete_by_template({'GS': '150', 'G_all': '150'}) == 83
    # Count them again, it should be zero
    assert len(appearances_rdb.find_by_template({'GS': '150', 'G_all': '150'})) == 0
...
def test_update_by_key(appearances_rdb):
    ...
    assert appearances_rdb.find_by_template({'playerID': 'aaronha01', 'teamID': 'ATL'}, ['yearID']) == [{'yearID': '1966'}, {'yearID': '1967'}, {'yearID': '1968'}, {'yearID': '1969'}, {'yearID': '1970'}, {'yearID': '1971'}, {'yearID': '1972'}, {'yearID': '1973'}, {'yearID': '1974'}]
    # Try change the record that has year '1970' to '1969', it should fail
    with pytest.raises(Exception):
        appearances_rdb.update_by_key(['aaronha01', 'ATL', '1970'], {'yearID': '1969'})
    # Try change the record that has year '1970' to '1900', it should succeed because '1900' was not amongst the result before.
    assert appearances_rdb.update_by_key(['aaronha01', 'ATL', '1970'], {'yearID': '1900'}) == 1

    # Example when record is not found (it has been updated already), return 0
    assert appearances_rdb.update_by_key(['aaronha01', 'ATL', '1970'], {'yearID': '1900'}) == 0

    # Function directly returns zero if new_values is None or empty, as nothing to update
    assert appearances_rdb.update_by_key(['aardsda01', 'ATL', '2015'], {}) == 0

    # Function fails if no key_fields are provided (None or empty) or if the dimension doesn't match with key_columns dimension
    with pytest.raises(Exception):
        appearances_rdb.update_by_key([], {})
    with pytest.raises(Exception):
...
def test_update_by_template(appearances_rdb):
    # Find some kind of appearances
    assert len(appearances_rdb.find_by_template({'teamID': 'ATL', 'yearID': '1970', 'G_p': '0'})) == 20
    # Try change 'G_p' from '0' to '1'
    assert appearances_rdb.update_by_template({'teamID': 'ATL', 'yearID': '1970', 'G_p': '0'}, {'G_p': '1'}) == 20
    # Find updated appearances with 'G_p' equals '1'
    assert len(appearances_rdb.find_by_template({'teamID': 'ATL', 'yearID': '1970', 'G_p': '1'})) == 20

    # Example when records are not found (they are updated already), return 0
    assert appearances_rdb.update_by_template({'teamID': 'ATL', 'yearID': '1970', 'G_p': '0'}, {'G_p': '1'}) == 0

    # Function directly returns zero if new_values is None or empty, as nothing to update
    assert appearances_rdb.update_by_template({'teamID': 'ATL', 'yearID': '1970', 'G_p': '1'}, {}) == 0
    ...
    assert appearances_rdb.update_by_template({}, {'G_p': '123'}) == 105789
    assert len(appearances_rdb.find_by_template({'G_p': '123'})) == 105789
